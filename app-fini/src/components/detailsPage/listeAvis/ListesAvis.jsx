...
const ListesAvis = ({ salle, id }) => {
  ...
  const handelSubmit = async () => {
    if (REGEX_AVIS.test(note) && note != "") {
      ...
      await axios
        .put(
          `${import.meta.env.VITE_REACT_APP_API_URL}Salles/${id}`,
          salle,
          config
        )
      ...
      {token ? (
        !addAvis ? (
          <div className={style.divBtn}>
            <button className={style.btn} onClick={() => setAddAvis(true)}>
              Ajouter Avis
            </button>
          </div>
        ) : (
          <div className={style.divBtn}>
            ...
            <div className={style.containerBtn}>
              <button className={style.btn} onClick={handelSubmit}>
                Ajouter
              </button>
              <button className={style.btn} onClick={() => setAddAvis(false)}>
                X
              </button>
            </div>
          </div>
